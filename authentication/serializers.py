...
class UserCreationSerializer(serializers.ModelSerializer):
    ...
    def validate(self,attrs):
        ...
        phonenumber=User.objects.filter(username=attrs.get('phone_number')).exists()

        if phonenumber:
            raise ValidationError(detail="User with phonenumber exists",code=status.HTTP_403_FORBIDDEN)
